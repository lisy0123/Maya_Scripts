...
import tabs.util
import functions.func_create

reload(tabs.util)
reload(functions.func_create)

from tabs.util import Utils
from functions.func_create import joint_size
...
class TabCreate():
    ...
    def frame_jnt_size(self):
        ...
        jnt = cmds.floatSliderButtonGrp(l="Size   ", bl="Set", bc="joint_size()", cw4=(50,50,70,50), f=True, min=0.1, max=1, v=0.5, h=30)
        cmds.setParent("..")
    ...
    def frame_trs(self):
        ...
        Utils().btn_layout(1)
        cmds.button(l="Match", c="match_freeze()", w=WI01[1], h=30)
        cmds.setParent("..")

        Utils().btn_layout(1)
        cmds.button(l="Freeze", c="match_freeze(False)", w=WI01[1], h=30)
        cmds.setParent("..")
        cmds.separator(h=1)
